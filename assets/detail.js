...
function loadDetail(data){
    console.log(data);
    document.getElementById('detail_image').src=data.url.regular;
    ...
    document.getElementById('download_link').href=data.links.dowload;

    const date=new Date(data.created_at);
    const upload_date=`${date.getUTCDate()}/${data.getUTCMonth()+1}/${date.getUTCFullYear()}`;
